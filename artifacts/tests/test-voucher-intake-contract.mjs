...
const simplify=fs.readFileSync("wordpress/casa-viva-dropship-core/assets/messenger-simplify.js","utf8");
for(const required of ["/api/messaging/parse-voucher","wp_remote_post","'rawVoucher' => $text","permission_callback","CVD_Registration::is_approved_gestora","cvd_nexo_unavailable","No se creó ningún pedido"]){if(!php.includes(required))throw new Error(`Falta contrato: ${required}`)}
if(!php.includes("'timeout' => 32"))throw new Error("El puente debe limitar la espera de NEXO sin dejar congelada la interfaz");
...
for(const required of ["normalizeSplitPayment","deliveryVoucherTotal","Pagador dividido propuesto"]){if(!fs.readFileSync("wordpress/casa-viva-dropship-core/assets/voucher-intake.js","utf8").includes(required))throw new Error(`Falta revisión de pagador dividido: ${required}`)}
...
if(!simplify.includes("reviewObserver?.disconnect()")||!simplify.includes("reviewObserver?.observe(review"))throw new Error("El observador de revisión puede autoactivarse y congelar la pantalla");
