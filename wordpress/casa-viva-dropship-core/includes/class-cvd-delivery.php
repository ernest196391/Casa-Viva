...
final class CVD_Delivery {
	...
	public static function change_status(): void {
		...
		if ( 'delivered' === $next && $is_messenger && ( ! isset( $_POST['money_confirmed'] ) || ! in_array( $collection_method, array( 'cash_usd', 'cash_cup', 'transfer', 'mixed', 'other' ), true ) || ( $collected_usd <= 0 && $collected_cup <= 0 ) ) ) { wp_die( 'Confirma el medio y los importes realmente recibidos antes de marcar la entrega.' ); }
		...
		if ( class_exists('CVD_Order_Transition_Service') && CVD_Order_Transition_Service::governs('delivery',$current,$next) ) {
			...
			$collection_metadata = 'delivered' === $next && $collection_method ? array( 'collection_method' => $collection_method, 'collected_usd' => $collected_usd, 'collected_cup' => $collected_cup ) : array();
