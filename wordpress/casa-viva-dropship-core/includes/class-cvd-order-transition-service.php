...
final class CVD_Order_Transition_Service {
	...
	private const DELIVERY_TRANSITIONS = array(
		...
		'assigned'=>array('accepted'), 'accepted'=>array('to_store'),
	);
	...
	public static function transition( int $order_id, string $domain, string $target_state, array $context=array() ): array {
		...
		try {
			...
			if(in_array($order->get_status(),array('completed','cancelled','refunded','failed'),true)){return self::failure(self::PRECONDITION_FAILED,$current);}
	...
	private static function authorized( WC_Order $order,string $domain,string $from,string $to,WP_User $actor ): bool {
		...
		if(in_array($to,array('offered','assigned'),true)){return $is_admin||$is_staff;}
	...
	private static function write_state_and_history(WC_Order $order,string $domain,string $from,string $to,int $actor_id,string $at,string $anchor,array $metadata):void{
		if('operation'===$domain){$history=$order->get_meta('_cvd_operation_history',true);$history=is_array($history)?$history:array();$history[]=array('from'=>$from,'to'=>$to,'user_id'=>$actor_id,'at'=>$at,'event_anchor'=>$anchor);$order->update_meta_data('_cvd_operation_status',$to);$order->update_meta_data('_cvd_operation_updated_at',$at);$order->update_meta_data('_cvd_operation_history',array_slice($history,-100));$order->add_order_note(sprintf('Operación Casa Viva: %s → %s.',self::label($from),self::label($to)));return;}
	...
	private static function delivery_label(string $status):string{$labels=array('unassigned'=>'Por ofertar','offered'=>'Oferta abierta','assigned'=>'Asignada','accepted'=>'Aceptada','to_store'=>'Va a recoger');return $labels[$status]??ucfirst($status);}
