(() => {
  ...
  async function choices(row, product, index) {
    ...
    const load = async () => { select.replaceChildren(Object.assign(document.createElement("option"), { value: "", textContent: "Selecciona el producto canónico" })); try { const response = await fetch(`${window.cvdVoucherIntake.productsEndpoint}?q=${encodeURIComponent(search.querySelector("input").value)}`, { credentials: "same-origin", headers: { "X-WP-Nonce": window.cvdVoucherIntake.nonce } }); for (const item of await response.json()) { const option = document.createElement("option"); option.value = String(item.id); option.textContent = `${item.name} · ${item.price} ${item.currency}`; select.append(option); } } catch { ui.status.textContent = "No se pudo consultar el catálogo. Reintenta."; } };
    search.querySelector("input").addEventListener("change", load); await load();
  }
