...
test('un catálogo lento no congela un vale ya interpretado', async ({ page }) => {
  await page.setViewportSize({ width: 360, height: 740 });
  await page.goto(`${baseURL}/wp-login.php`);
  await page.locator('#user_login').fill(operator.user);
  await page.locator('#user_pass').fill(operator.pass);
  ...
  await page.locator('#cvd-voucher-text').fill('Vale sintético completo para comprobar que un catálogo lento no bloquea la revisión del operador.');
  await page.locator('[data-voucher-parse]').click();
